jQuery(function ($) {
            ...
            $.ajax({
                ...
                error: function (jqXHR, textStatus, errorThrown) {
                    ...
                    if (responseText.message == "CSRF token mismatch.") {
                        if (settings.native_api)
                            Website2APK.showToast(
                                "Session Timed Out! Reloading.."
                            );
                        else alert("Session Timed Out! Reloading..");

                        location.reload();
                        return false;
                    }

                    if (settings.native_api) Website2APK.showToast(errorThrown);
                ...
                remote: {
                    ...
                    complete: function (data) {
                        ...
                        if (data === true) {
                            ...
                            sys_alert("Client already added!");
                            return false;
                        }
    ...
    function sys_alert(text) {
        if (settings.native_api) Website2APK.showToast(text);
        else alert(text);
    }
